...
function harness(user=users[0],options={}) {
 const store={TeamMember:clone(options.members||baseMembers),TodoTask:clone(options.tasks||[task('g1'),task('g2'),task('i1','mi')]),User:clone(users),Jobs:clone(options.jobs||[{id:'job-1',canonical_name:'Acme Lot 1'}])};
...
test('optional job association preserves old tasks and current crew permissions',async()=>{const h=harness(users[2]);let board=await h.call({action:'board'});assert.equal(board.status,200);assert.ok(board.body.tasks.every(t=>t.job_id===undefined));let made=await h.call({action:'create',title:'Job task',details:'',category:'follow_up',job_id:'job-1',request_key:'request-job-link-000001'});assert.equal(made.status,200);assert.equal(made.body.task.job_id,'job-1');let changed=await h.call({action:'update_task',id:'i1',expected_revision:0,patch:{job_id:'job-1'}});assert.equal(changed.status,200);assert.equal(changed.body.task.job_id,'job-1');assert.equal((await h.call({action:'update_task',id:'g1',expected_revision:0,patch:{job_id:'job-1'}})).status,404);assert.equal((await h.call({action:'create',title:'Bad job',job_id:'missing',request_key:'request-job-link-000002'})).status,400);});
