...
test("resolved non-tempered and pane construction appear without creating explicit choices",()=>{
 ...
 assert.equal(automaticOptionLabel("super_spacer",line,settings,price),"No (automatic)");
 assert.equal(automaticOptionLabel("glass_thickness",line,settings,price),"Double-strength over double-strength (DS/DS) (automatic)");
...
test("quote defaults, explicit overrides and fractional pane descriptions retain their meaning",()=>{
 ...
 assert.equal(automaticOptionLabel("tempered",explicit,{tempered:false}),"Non-tempered glass (quote default)");
