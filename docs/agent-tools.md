...
|---|---|---|---|---|
| `gf-gmail` | whichever Google account authorized the `gmail` connector (seeded as gabriel.fronk.wd@gmail.com; currently gabefronk@gmail.com) | Gmail (`gmail` connector, gmail.modify) | **owner** — only gabefronk@gmail.com / gabriel.fronk.wd@gmail.com see its entries | Each run checks the connector's profile and rewrites the row's `address` to the authorized account (warned in the run). gabefronk@gmail.com is also filter-forwarded to gabriel.fronk.wd@gmail.com; each entry records the address the mail was really sent to as `account_hint` (X-Forwarded-For / Delivered-To / X-Original-To / To). |
