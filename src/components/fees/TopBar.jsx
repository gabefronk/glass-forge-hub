...
export default function TopBar({ month, onMonthChange, onExport, onAddSplit, topRef,
  ...
  return (
    <div ref={topRef} style={{ backgroundColor: C.pageBg, borderBottom: `1px solid ${C.border}` }}>
      {/* Row 1: month nav + actions */}
      <div className="px-[26px] max-[699px]:px-[18px] py-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <button onClick={prev} aria-label="Previous month" className="h-10 w-10 shrink-0 rounded-full flex items-center justify-center transition-colors hover:bg-white/5" style={{ backgroundColor: "rgba(255,255,255,.07)" }}>
          ...
          <button onClick={next} aria-label="Next month" className="h-10 w-10 shrink-0 rounded-full flex items-center justify-center transition-colors hover:bg-white/5" style={{ backgroundColor: "rgba(255,255,255,.07)" }}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={onAddSplit} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full text-[12px] font-semibold whitespace-nowrap" style={{ backgroundColor: C.accent, color: C.accentDark }}>
            ...
            <span>Add profit-split job</span>
          </button>
        </div>
      </div>
      ...
      <div className="px-[26px] max-[699px]:px-[18px] pb-4">
        ...
        <div className="md:hidden mb-3">
          <div className="rounded-[16px] p-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
            ...
            <div className="font-mono-num-bold text-[clamp(24px,7vw,32px)] break-words" style={{ color: C.accent, letterSpacing: "-0.03em" }}>${formatMoney(notYetBilledTotal)}</div>
...
function MetricCard({ label, value, sub, valueColor }) {
  return (
    <div className="min-w-0 rounded-[14px] p-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      <div className="mono-label-sm mb-2">{label}</div>
      <div className="font-mono-num-bold text-[20px] break-words" style={{ color: valueColor || C.text, letterSpacing: "-0.025em" }}>
        {value}
      </div>
...
function MiniMetric({ label, value, valueColor }) {
  return (
    <div className="rounded-[10px] px-2 py-1.5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      ...
      <div className="font-mono-num text-[13px] break-words" style={{ color: valueColor || C.text }}>{value}</div>
    </div>
  );
}
