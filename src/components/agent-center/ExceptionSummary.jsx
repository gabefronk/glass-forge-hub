...
export default function ExceptionSummary({ runs }) {
  ...
  return (
    <div className="rounded-2xl border border-[#B8CBE8] bg-[#F4F8FE] p-5">
      ...
      <p className="mt-1 text-xs text-slate-600">Combined for Glass Forge manager → Operations Director. Only blocked or skipped section runs appear; completed sections are not escalated.</p>
      {exceptions.length === 0 ? (
        ...
        <ul className="mt-3 space-y-2">
          {exceptions.map(r => (
            <li key={r.id} className="rounded-lg border border-[#DDE3EC] bg-white p-3 text-sm">
              ...
              {r.blockers.length > 0 && <p className="mt-1 text-xs text-amber-800"><span className="font-semibold">Blockers:</span> {r.blockers.join("; ")}</p>}
              <p className="mt-1 text-xs text-slate-400">{formatTime(r.completedAt || r.startedAt)}</p>
            </li>
          ))}
        </ul>
      )}
