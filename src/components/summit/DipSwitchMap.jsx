...
import { DIP_SWITCHES, SUMMIT_CARD_SHADOW, SUMMIT_META } from "./summitData";
import PhotoSlot from "./PhotoSlot";

// Visual diagram of the 8 DIP switches. Function names + source come from
// SUMMIT_META.dip_switch_functions; tap-detail descriptions/warnings from DIP_SWITCHES.
export default function DipSwitchMap() {
  const [selected, setSelected] = useState(null);
  const dip = SUMMIT_META.dip_switch_functions;
  const detailFor = (n) => DIP_SWITCHES.find((d) => d.number === n) || {};
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-[14px] p-5" style={{ border: `1px solid ${C.border}`, backgroundColor: C.card, boxShadow: SUMMIT_CARD_SHADOW }}>
        ...
        <p className="text-[13px] mt-1" style={{ color: C.textSecondary }}>Tap a switch for its function and warning.</p>
        ...
        <div className="mt-4 rounded-[10px] px-3.5 py-3" style={{ border: `1px solid ${C.border}`, backgroundColor: C.cardAlt }}>
          <p className="text-[12px] leading-snug" style={{ color: C.textSecondary }}>{dip.source}</p>
        </div>
        ...
        <div className="mt-4 flex items-end justify-between gap-1.5 overflow-x-auto obsidian-scroll pb-1">
          {DIP_SWITCHES.map((d) => (
            <button
              ...
              aria-label={`DIP switch ${d.number}: ${dip[d.number] || d.name}`}
              className="flex flex-col items-center gap-2 shrink-0"
              style={{ minWidth: "44px" }}
            >
              <div
                className="rounded-[6px] flex items-center justify-center font-mono-num text-[14px] font-bold"
                style={{ width: "44px", height: "64px", border: `1px solid ${C.borderStrong}`, backgroundColor: C.cardAlt, color: C.text }}
              >
                {d.number}
              </div>
              <span className="text-[9.5px] font-mono uppercase tracking-[0.04em] text-center leading-tight" style={{ color: C.textMuted }}>
                {(dip[d.number] || d.name).split(" - ")[0]}
              </span>
            </button>
          ))}
        </div>
          ...
          >
            <div className="flex items-start justify-between gap-3 px-5 pt-4 pb-2">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <h4 className="font-heading text-[17px] font-bold leading-tight" style={{ color: C.text }}>{dip[selected.number] || selected.name}</h4>
              </div>
            ...
            <div className="px-5 pb-4">
              <p className="text-[14px] leading-snug" style={{ color: C.textSecondary }}>{detailFor(selected.number).description || selected.description}</p>
              <div className="mt-3 rounded-[10px] px-3.5 py-3 flex items-start gap-2.5" style={{ border: `1px solid ${C.border}`, backgroundColor: C.amberLight }}>
                ...
                <p className="text-[13px] leading-snug" style={{ color: C.text }}>{detailFor(selected.number).warning || selected.warning}</p>
              </div>
            </div>
          </div>
        </>
