...
export default function EventBubble({ event, jobs, onEdit, onDelete, onClose, saving, user, onChanged }) {
  ...
  if (editing && event.source === "app") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(24,36,34,.40)" }} onClick={onClose}>
        <div className="rounded-[14px] max-w-2xl w-full max-h-[90dvh] overflow-y-auto obsidian-scroll card-shadow" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }} onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: `1px solid ${C.border}` }}>
            ...
            <button onClick={onClose} aria-label="Close event" className="inline-flex h-11 w-11 items-center justify-center rounded-full transition-colors hover:bg-[#F8F9F6]"><X className="h-4 w-4" style={{ color: C.textMuted }} /></button>
          </div>
          <div className="p-4">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(24,36,34,.40)" }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-end gap-1 px-3 pt-3">
          {!readOnly && (
            <>
              <button onClick={() => setEditing(true)} className="inline-flex h-11 w-11 items-center justify-center rounded-full transition-colors hover:bg-[#F8F9F6]" title="Edit">
                <Pencil className="h-4 w-4" style={{ color: C.textMuted }} />
              </button>
              <button onClick={onDelete} className="inline-flex h-11 w-11 items-center justify-center rounded-full transition-colors hover:bg-[#F8F9F6]" title="Delete">
                <Trash2 className="h-4 w-4" style={{ color: C.textMuted }} />
              </button>
            </>
          )}
          <button onClick={onClose} className="inline-flex h-11 w-11 items-center justify-center rounded-full transition-colors hover:bg-[#F8F9F6]" title="Close">
        ...
        <div className="px-5 pb-4 pt-2 space-y-1">
          {readOnly && (
            <div className="flex items-center gap-1.5 text-[12px] rounded-[10px] px-2.5 py-2 mb-2" style={{ backgroundColor: "#FCEDEC", border: "1px solid #F0C9C5", color: "#A43432" }}>
