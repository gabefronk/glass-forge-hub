  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="break-words font-semibold">{event.job_name}</div>
      ...
      {event.scope_notes && (
        <div>
          ...
          <div className="whitespace-pre-wrap break-words text-sm">{event.scope_notes}</div>
        </div>
      )}
      {event.prerequisites && (
        <div>
          <div className="text-xs text-muted-foreground mb-1">Prerequisites</div>
          <div className="whitespace-pre-wrap break-words text-sm">{event.prerequisites}</div>
        </div>
      )}
      {!readOnly && (
