...
const selectCls = "w-full min-w-0 min-h-11 rounded-md border border-input bg-background px-3 py-2 text-base sm:text-sm";
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(f); }} className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Field label="Job name" className="sm:col-span-2"><Input required value={f.job_name} onChange={set("job_name")} /></Field>
        ...
        <Field label="Scope notes (crew-safe)" className="sm:col-span-2"><Textarea rows={3} value={f.scope_notes} onChange={set("scope_notes")} /></Field>
        <Field label="Prerequisites (forklift, access, materials)" className="sm:col-span-2"><Textarea rows={2} value={f.prerequisites} onChange={set("prerequisites")} /></Field>
      </div>
