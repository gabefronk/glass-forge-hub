...
Use the dealer, shipping yard and gross margin I provide; ask if any are missing. Create a fresh draft with invoke_submit_window_takeoff (action: create, request_id, title, message, settings, lines, source). Use a stable request_id when retrying. For automatic quoting, use Studio 1 3/8 inch Fin Setback Single Hung, Call dimensions in inches, BFS / BFS-UTAH DESIGN(11), White or Taupe, and CozE LowE. Supply every per-line specification, or have me choose the standard Studio profile in Window Quotes. Do not claim I confirmed a profile unless I actually did. Unsupported products stay for review. When the required details are complete, use invoke_queue_window_quote (action: queue, quote_id) and the quoting computer will pick it up automatically. Check invoke_get_window_quote_status (action: detail, quote_id) when I ask for progress; use invoke_reply_window_quote for answers or invoke_update_window_takeoff to correct the schedule before queueing again. Report the real saved status and verified AMSCO result. Keep this in Window Quotes; no Job is created before I mark the sale Won.`;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[90dvh] max-w-xl overflow-y-auto rounded-2xl bg-white">
        ...
        <ol className="list-decimal space-y-3 pl-5 text-sm leading-relaxed text-[#535E72]">
          ...
          <li>Use your Bluebeam tools in Claude to prepare the takeoff. If a quote tool is missing, refresh the Glass Forge connection.</li>
        </ol>
