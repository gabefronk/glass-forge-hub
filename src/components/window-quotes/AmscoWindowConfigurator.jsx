...
function SelectRow({ label, value, choices, onChange, note, formatValue = String }) {
...
export default function AmscoWindowConfigurator({ line, index, settings, disabled, tradeCode, onTradeCode, onApplyTradeCode, onChange, onOption, onCancel, onSave, price, priceStatus }) {
  ...
  return <section className="min-w-0 rounded-lg border border-[#bdcbd4] bg-white shadow-sm" aria-label="AMSCO window configurator">
    ...
    <label className="flex items-center gap-3 border-b border-[#c8d3da] bg-[#edf1f4] px-4 py-3 sm:hidden"><span className="text-xs font-semibold text-[#426172]">Section</span><select aria-label="Configuration section" className={input} value={step} disabled={disabled} onChange={e => chooseStep(Number(e.target.value))}>{steps.map((name,n)=><option key={name} value={n}>{n+1}. {name}</option>)}</select></label>
    <div ref={stepRef} role="tablist" aria-label="Window configuration steps" className="hidden min-w-0 flex-wrap border-b border-[#c8d3da] bg-[#edf1f4] sm:flex">{steps.map((label, n) => <button type="button" role="tab" id={uid + "-tab-" + n} aria-controls={uid + "-panel"} aria-selected={step === n} data-step={n} key={label} disabled={disabled} onClick={() => chooseStep(n)} className={"inline-flex min-h-11 shrink-0 items-center gap-2 border-r border-[#c8d3da] px-3 py-2 text-left text-xs font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#196c86] " + (step === n ? "bg-[#196c86] text-white" : "text-[#426172] hover:bg-[#dfe9ef]")}><span className={"flex h-5 w-5 items-center justify-center rounded-full text-[10px] " + (step === n ? "bg-white/20" : "bg-white")}>{n + 1}</span><span className="max-w-28">{label}</span><ChevronRight size={13} /></button>)}</div>
    <div className="grid min-w-0 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <div className="min-w-0" role="tabpanel" id={uid + "-panel"} aria-labelledby={uid + "-tab-" + step}><h3 className="border-b border-[#d9e2e8] px-4 py-4 text-base font-semibold text-[#305367]">{steps[step]}</h3><fieldset disabled={disabled} className="min-w-0"><legend className="sr-only">{steps[step]}</legend>
        ...
        {step === 2 && <>
          ...
          {boolRow("Glass Tempering", "tempered")}{optionRow("Glass Coating", "glass", ["CozE (LowE)"])}{optionRow("Glass Thickness", "glass_thickness", GLASS_THICKNESS_CHOICES)}{!specifications.glass_thickness && <Note>Pane thickness has not been returned for this size. It will appear here when AMSCO resolves the construction.</Note>}{optionRow("Patterned Glass", "patterned_glass", ["None", "Obscure"])}{boolRow("Argon", "argon")}{optionRow("Elevation", "elevation", ["None", "Above 8001", "6501 to 8000", "2501 to 6500", "1001 to 2500", "Below 1000"])}{boolRow("Super Spacer", "super_spacer")}
        </>}
        {step === 3 && <><Note>Automatic selections show the returned specification. A field marked unresolved is still awaiting its construction value.</Note>{optionRow("Glazing Method", "glazing_method", [{ value: "3/4 Insulated", label: "3/4″ insulated glass unit" }])}{boolRow("Capillary Tubes", "capillary_tubes")}{optionRow("Hardware Color", "hardware_color", ["White", "Taupe", "Black"])}{optionRow("Screen", "screen", ["White", "Taupe", "Black", "None"])}</>}
        {step === 4 && <>
          <SelectRow label="Grille Pattern" value={customGrille ? "custom" : grilles.mode} choices={[{ value: "standard", label: automatic("grilles") }, { value: "none", label: "None" }, { value: "rectangular", label: "Rectangular" }, { value: "custom", label: "Custom / existing specification" }]} onChange={mode => { setCustomGrille(mode === "custom"); if (mode !== "custom") updateGrilles({ mode }); }} />
        ...
        {step === 5 && <>{optionRow("Installation Fin", "fin", ["Nail Fin", "Flush Fin"])}<Note>The frame series determines the standard installation setup. Explicit installation requirements are retained for AMSCO to verify.</Note></>}
        ...
        {step === 7 && <Note>{glassSummary || "Enter the window selections to resolve the glass specification."}{!specifications.glass_thickness && " Pane thickness is awaiting AMSCO."}</Note>}
