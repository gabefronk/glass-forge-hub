    ...
    <div className="grid min-w-0 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <div className="min-w-0" role="tabpanel" id={uid + "-panel"} aria-labelledby={uid + "-tab-" + step}><h3 className="border-b border-[#d9e2e8] px-4 py-4 text-base font-semibold text-[#305367]">{steps[step]}</h3><fieldset disabled={disabled} className="min-w-0"><legend className="sr-only">{steps[step]}</legend>
        ...
        {step === 2 && <>
          ...
          {boolRow("Glass Tempering", "tempered")}{optionRow("Glass Coating", "glass", ["CozE (LowE)"])}{optionRow("Glass Thickness", "glass_thickness", GLASS_THICKNESS_CHOICES)}{!specifications.glass_thickness && <Note>The current price calculation does not include pane thickness.</Note>}{optionRow("Patterned Glass", "patterned_glass", ["None", "Obscure"])}{boolRow("Argon", "argon")}{optionRow("Elevation", "elevation", ["None", "Above 8001", "6501 to 8000", "2501 to 6500", "1001 to 2500", "Below 1000"])}{boolRow("Super Spacer", "super_spacer")}
        </>}
        {step === 3 && <><Note>Automatic selections show the returned specification. Unavailable fields are not included in the current pricing result.</Note>{optionRow("Glazing Method", "glazing_method", [{ value: "3/4 Insulated", label: "3/4″ insulated glass unit" }])}{boolRow("Capillary Tubes", "capillary_tubes")}{optionRow("Hardware Color", "hardware_color", ["White", "Taupe", "Black"])}{optionRow("Screen", "screen", ["White", "Taupe", "Black", "None"])}</>}
        {step === 4 && <>
        ...
        {step === 7 && <Note>{glassSummary || "Enter the window selections to resolve the glass specification."}{!specifications.glass_thickness && " Pane thickness is not included in the current pricing result."}</Note>}
