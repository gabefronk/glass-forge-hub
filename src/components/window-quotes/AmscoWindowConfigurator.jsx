  ...
  return <section className="min-w-0 rounded-lg border border-[#bdcbd4] bg-white shadow-sm" aria-label="AMSCO window configurator">
    ...
    <div className="grid min-w-0 lg:grid-cols-[minmax(0,.75fr)_minmax(0,1.25fr)]">
      <aside className="min-w-0 border-b border-[#d9e2e8] bg-[#fafcfd] px-4 py-4 lg:border-b-0 lg:border-r" aria-label="Window preview">
        ...
        <div className="mt-4 border-t border-[#d9e2e8] pt-3" aria-live="polite"><p className="text-xs font-semibold text-[#526d7d]">Customer price · each</p>{priceStatus === "loading" ? <p className="mt-2 flex items-center gap-2 text-sm text-[#6e8390]"><Loader2 size={15} className="animate-spin" />Checking price…</p> : price?.status === "priced" ? <><p className="mt-1 text-2xl font-semibold text-[#286a54]">{money(price.unit_prices?.customer)}</p><p className="mt-1 text-xs text-[#6e8390]">Previously verified configuration · {money(price.line_totals?.customer)} for this quantity</p></> : <p className="mt-2 text-sm leading-relaxed text-[#687e8b]">{price?.status === "amsco_lookup_needed" ? "These selections need an AMSCO online quote." : price?.status === "native_calculation_needed" ? "Supported by the AMSCO pricing engine. Save the quote to calculate." : priceStatus === "unavailable" ? "Price check unavailable. Your selections are still here." : "Choose the product and enter its size."}</p>}</div>
