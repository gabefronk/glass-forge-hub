...
export default function ImportedAmscoQuote({snapshot,preview=false,importedAt}) {
  ...
  return <div className="min-w-0 space-y-4">
    ...
    {tab==="details"&&<div className="space-y-6 rounded-lg border border-[#CBD5E2] bg-white p-4"><Fields fields={snapshot.details}/><div><h3 className="mb-3 font-semibold text-[#305367]">Saved quote totals</h3><dl className="space-y-3 text-sm">{[["Customer subtotal","subtotal"],["Tax","tax"],["Labor","labor"],["Freight","freight"],["Shipping","shipping"],["Handling","handling"],["Miscellaneous","misc"],["Other quote adjustments","unitemized_adjustment"],["Discount","discount"],["Customer total","customer_total"],["Dealer total","dealer_total"]].filter(([,key])=>totals[key]!=null).map(([label,key])=><div key={key} className={"flex justify-between gap-3 "+(key==="customer_total"?"border-t pt-3 font-semibold":"")}><dt>{label}</dt><dd className="tabular-nums">{money(totals[key])}</dd></div>)}</dl></div><p className="text-xs leading-relaxed text-[#77839A]">These are the values saved in AMSCO when the quote was read.</p></div>}
