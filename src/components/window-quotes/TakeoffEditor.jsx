...
export const inputClass = "w-full min-w-0 min-h-11 rounded-lg border border-[#CBD4E1] bg-white px-3 py-2 text-base sm:text-sm text-[#131A26] outline-none focus:border-[#2A5EA8] focus:ring-2 focus:ring-[#E7EEFA] disabled:bg-[#F6F8FC] disabled:text-[#77839A]";
export const secondaryClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#DDE3EC] bg-white px-3 py-2 text-sm font-medium text-[#535E72] hover:bg-[#F6F8FC] disabled:opacity-50 disabled:cursor-not-allowed";
    ...
    <div className="space-y-2 sm:max-h-[440px] sm:overflow-y-auto sm:pr-1">
      {lines.map((line, index) => <details key={index} open={lines.length === 1 || undefined} className="group rounded-xl border border-[#DDE3EC] bg-white">
        <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-3 text-sm">
          ...
          <span className="min-w-0 flex-1"><span className="block truncate font-medium text-[#131A26]">{line.mark ? `${line.mark} · ` : ""}{line.style || "Window details needed"}</span><span className="block break-words text-xs text-[#616D81]">{line.width || "—"} × {line.height || "—"} {line.units || "units?"} · Qty {line.qty || "—"} · {(line.dimension_basis || "Choose dimension basis").replaceAll("_", " ")}</span></span>
        ...
        <div className="space-y-3 border-t border-[#E9EDF4] px-3 py-3">
          ...
          <button type="button" className="inline-flex min-h-11 items-center gap-1.5 text-xs font-medium text-[#8A4038]" disabled={disabled} onClick={() => onChange(lines.filter((_, i) => i !== index))}><Trash2 size={13} />Remove line</button>
        </div>
      </details>)}
    </div>
