...
function WindowSheet({ line, index, currency, drawing, scale }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-[#DDE3EC] bg-white print:break-inside-avoid">
    ...
    <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,260px),1fr))] items-start gap-5 p-4 sm:p-5">
      <WindowSchematic line={line} drawing={drawing} scale={scale} />
      <div className="min-w-0 space-y-5">
        <dl className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-4 rounded-xl bg-[#F6F8FC] p-4">
        ...
        <section><h5 className="mb-3 text-xs font-semibold text-[#535E72]">Saved configuration</h5>{options.length ? <dl className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-x-4 gap-y-3">{options.map(([label, value]) => <Specification key={label} label={label} value={value} />)}</dl> : <p className="text-xs text-[#616D81]">Product options were not supplied in this result.</p>}</section>
        {native.length > 0 && <details className="border-t border-[#E9EDF4] pt-3"><summary className="cursor-pointer text-xs font-semibold text-[#535E72]">Native configuration values</summary><dl className="mt-3 grid grid-cols-1 min-[400px]:grid-cols-2 gap-x-4 gap-y-3">{native.map(([label, value]) => <Specification key={label} label={label} value={value} />)}</dl></details>}
      </div>
    </div>
    {line.pricing_evidence?.source === "amsco_source_engine" && <details className="border-t border-[#E9EDF4] px-5 py-3"><summary className="min-h-11 cursor-pointer py-3 text-xs font-semibold text-[#305367]">AMSCO pricebook breakdown</summary><dl className="space-y-2 text-xs">{(line.pricing_evidence.components || []).map((component,n)=><div key={n} className="flex justify-between gap-3"><dt className="text-[#616D81]">{component.name}</dt><dd>{money(component.value,currency)}</dd></div>)}</dl><p className="mt-3 text-[11px] text-[#77839A]">List-price components calculated from the AMSCO pricebook; dealer discount and your margin are applied below.</p></details>}
    <div className="border-t border-[#E9EDF4] bg-[#FAFBFD] px-5 py-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-[#77839A]">Customer unit price</p><p className="mt-1 break-all text-base font-semibold tabular-nums text-[#131A26]">{money(unitPrice, currency)}</p></div><div className="sm:text-right"><p className="text-[10px] font-semibold uppercase tracking-wide text-[#77839A]">Line total · Qty {display(qty)}</p><p className="mt-1 break-all text-xl font-semibold tabular-nums text-[#1E4A85]">{money(extendedPrice, currency)}</p></div></div>
      {(costRows.length > 0 || present(line.gross_margin)) && <details className="mt-3 border-t border-[#E9EDF4] pt-3"><summary className="cursor-pointer text-xs font-medium text-[#616D81]">Cost and margin</summary><div className="mt-3 space-y-2 overflow-x-auto text-xs text-[#535E72]">{costRows.length > 0 && <table className="w-full min-w-[250px] text-right"><thead className="text-[10px] text-[#77839A]"><tr><th className="pb-2 text-left font-medium">Price basis</th><th className="pb-2 font-medium">Per unit</th><th className="pb-2 font-medium">Extended</th></tr></thead><tbody>{costRows.map(([label, unit, total]) => <tr key={label}><th className="py-1 text-left font-medium">{label}</th><td className="py-1 tabular-nums">{money(unit, currency)}</td><td className="py-1 tabular-nums">{money(total, currency)}</td></tr>)}</tbody></table>}{present(line.gross_margin) && <p>Gross margin <span className="font-semibold text-[#131A26]">{display(line.gross_margin)}%</span></p>}</div></details>}
    </div>
  </article>;
}
