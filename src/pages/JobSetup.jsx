...
import { buildSetupDraft, computeSetupTotals, toContractViewModel } from "@/lib/jobSetup";
...
function JobSetupOwner({ user }) {
  ...
  async function save(patch = {}) {
    ...
    if (next.status === "approved" && (!String(next.approver_name || "").trim() || !next.approved_date)) {
      setMessage("Approver name and approved date are required before approval.");
      return;
    }
    setBusy(true); setMessage("");
    try {
  ...
  if (contract) return <ContractPage model={toContractViewModel(sheet, job)} onBack={() => setContract(false)} />;
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 sm:px-7">
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-5 grid gap-5">
        ...
        <Card title="Budget & pricing" note="Uses the existing Window Budget Sheet formulas (7.45% tax, 30% material margin, 27% labor margin). Source labels show where prefilled values came from.">
