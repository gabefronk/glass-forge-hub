...
export default function EmailInbox() {
  ...
  const loadMailboxes = async () => { const r = await emailCall({ action: "mailboxes" }); boxesLoaded.current = true; setMailboxes(Array.isArray(r.mailboxes) ? r.mailboxes : []); };
  ...
  const refresh = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      ...
      const [list] = await Promise.all([emailCall(payload), boxesLoaded.current ? Promise.resolve() : loadMailboxes()]);
      if (mine !== seq.current) return;
      setThreads(Array.isArray(list.threads) ? list.threads : []);
      if (Array.isArray(list.mailboxes)) setMailboxes(list.mailboxes);
      setError("");
    } catch (e) {
      if (mine === seq.current) { setError(errorText(e, "Email could not load. Reload to try again.")); if (!boxesLoaded.current) { boxesLoaded.current = true; setMailboxes([]); } }
  ...
  // /email?thread=<id>: show that thread even when it falls outside the loaded list.
  useEffect(() => {
    if (!wanted || loading || pinned?.id === wanted) return undefined;
    if (threads.some((t) => t.id === wanted)) { setPinned(null); return undefined; }
    let active = true;
  ...
  const toggle = (id) => {
    ...
    setExpandedId(next);
    if (!next && pinned?.id === id) setPinned(null);
    const p = new URLSearchParams(searchParams);
    if (next) p.set("thread", next); else p.delete("thread");
    setSearchParams(p, { replace: true });
  };
  const clearFilters = () => { setStatus("open"); setCategory(""); setQ(""); };
  ...
  const seed = async () => {
    ...
    try { await emailCall({ action: "seed_mailboxes" }); await loadMailboxes(); setNotice("Mailboxes created. Authorize each one from the Base44 integrations page, then sync."); }
  ...
  const emptyText = threads.length ? "No threads match these filters." : mailbox !== "all" && unconnected.length ? "Nothing here until this mailbox is connected." : "Nothing in the inbox yet. Run a sync once a mailbox is connected.";
      ...
      {mailboxes?.length ? (
        <>
          <section className="card-shadow flex flex-col gap-3 rounded-[14px] bg-white p-3" style={{ border: `1px solid ${C.border}` }} aria-label="Filters">
            <div className="flex flex-col gap-2 sm:flex-row">
              <label className="relative flex min-w-0 flex-1 items-center">
                ...
                <input type="search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search subject, sender, job" aria-label="Search email" className="min-h-11 w-full rounded-[9px] bg-white pl-[38px] pr-9 text-[14px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--gf-teal-500)]" style={field} />
          ...
          <div className="flex flex-col gap-2.5">
            ...
            {loading && !visible.length ? <p role="status" className="p-6 text-center text-[13px]" style={{ color: C.textSecondary }}>Loading threads…</p> : null}
