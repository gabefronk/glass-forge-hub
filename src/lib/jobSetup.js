...
export const PLACEHOLDER_TERMS = Object.freeze({
  deposit_pct: 50,
  payment_schedule: "PLACEHOLDER — 50% deposit, balance due at substantial completion. Gabriel: edit before approval.",
  estimated_lead_time: "PLACEHOLDER — confirm manufacturer lead time.",
  estimated_start: "",
  warranty_text: "PLACEHOLDER — add the applicable product and workmanship warranty.",
  quote_valid_days: 30,
});
...
export function buildSetupDraft({ job = {}, budget = {}, costInput = {}, installBudget = {} } = {}) {
  ...
  return {
    ...
    sources: {
      ...
      tax: present(tax) ? "Accepted quote" : "Job budget (7.45%)",
    },
...
export function computeSetupTotals(sheet = {}) {
  ...
  const tax = finite(p.tax) ? Number(p.tax) : hasBudgetData ? budget.use_tax : "";
