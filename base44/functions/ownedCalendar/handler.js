...
export function createOwnedCalendarHandler({getClient,readTracker}) {
 return async req => {
  ...
  try {
   ...
   return Response.json({groups:result.groups,ownership:{counts:result.counts,by_month:result.by_month,tracker_captured_at:tracker.snapshot.source_captured_at,tracker_rows:tracker.rows.length,appended_rows:tracker.appended?.length||0},
    ...
    ...(user.role==="admin"?{excluded_events:result.rejected}:{}),generated_at:new Date().toISOString()});
