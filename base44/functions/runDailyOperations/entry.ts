...
// base44/shared/pagination.ts
async function fetchAllPages(entity, sort = "-created_date", pageSize = 1e3) {
...
// base44/shared/agentOpsRoles.js
var SECTION_LEADS = [
  { id: "calendar_ops_lead", name: "Calendar Operations Lead", department: "Operations \xB7 Lead", label: "Calendar Operations", connector: "googlecalendar", connectorLabel: "Google Calendar", plan: [
  ...
  { id: "sales_order_lead", name: "Sales & Order Lead", department: "Operations \xB7 Lead", label: "Sales & Order", connector: null, plan: [
  ...
  { id: "field_reporting_lead", name: "Field Reporting Lead", department: "Operations \xB7 Lead", label: "Field Reporting", connector: "probuild", connectorLabel: "ProBuild", plan: [
  ...
  { id: "quoting_lead", name: "Quoting Lead", department: "Product Engineering \xB7 Lead", label: "Quoting", connector: "superagent", connectorLabel: "External Claude quoting", plan: [
  ...
  { id: "development_lead", name: "Development Lead", department: "Product Engineering \xB7 Lead", label: "Development", connector: null, plan: [
...
var DEFAULT_ENABLEMENT = Object.fromEntries(SECTION_LEADS.map((l) => [l.id, true]));
var MISSING_REPORT_STATES = ["missing_all", "missing_photos", "missing_notes"];
var KNOWLEDGE_LEADS = ["calendar_ops_lead", "sales_order_lead", "field_reporting_lead", "development_lead"];
var SOURCE_ROLES = {
...
var nowFor = (ctx) => {
  const value = typeof ctx.now === "function" ? ctx.now() : ctx.now || /* @__PURE__ */ new Date();
  const now = value instanceof Date ? value : new Date(value);
  if (!Number.isFinite(now.getTime())) throw Error("Invalid daily operations clock.");
  return now;
};
var readOnce = (ctx, key, action) => {
...
var calendarRows = (ctx) => readOnce(ctx, "calendar_rows", () => fetchAllPages(ctx.entities.CalendarEvents, "-event_date", 1e3));
var pastRequired = (events, today) => events.filter((event) => typeof event.event_date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(event.event_date) && event.event_date <= today && event.source_status !== "cancelled" && event.report_required !== false);
var sourceLabel = (key) => ({ live_google: "Google Calendar live read", outlook_installation: "Outlook installation capture", outlook_service: "Outlook service capture", sales_tracker: "Sales Tracker capture", live_probuild: "ProBuild incremental live read", probuild_library: "ProBuild saved library", documents: "Document evidence" })[key] || key.replaceAll("_", " ");
function sourceReview(key, source, nowMs) {
  ...
  if (!Number.isFinite(checked) || checked > nowMs + 3e5) return { text: label + ": source freshness unverified", gap: true };
  if (nowMs - checked > 26 * 36e5) return { text: label + ": stale capture (" + new Date(checked).toISOString() + ")", gap: true };
...
async function knowledgeReview(lead, ctx) {
  ...
  if (!Number.isFinite(checked) || checked > nowMs + 3e5 || nowMs - checked > 26 * 36e5) exceptions.push("The latest completed job preparation is stale or has an invalid completion time.");
  const keys2 = SOURCE_ROLES[lead.id] || Object.keys(run.source_status || {});
  const sources = keys2.map((key) => sourceReview(key, run.source_status?.[key], nowMs));
  ...
  const issues = (Array.isArray(run.issues) ? run.issues : []).filter((issue2) => issue2 && !["resolved", "closed"].includes(issue2.status) && (lead.id === "development_lead" || issue2.assigned_to === lead.id));
  const codes = [...new Set(issues.map((issue2) => /^[a-z0-9_:-]{1,100}$/i.test(String(issue2.code || "")) ? issue2.code.replaceAll("_", " ") : "source review required"))];
...
async function checkConnector(kind, ctx) {
  try {
...
async function reconcileLead(lead, ctx) {
  const exceptions = [];
  let summary = "";
  ...
  try {
    if (lead.id === "calendar_ops_lead") {
      ...
      const reports = await readOnce(ctx, "field_reports", () => fetchAllPages(ctx.entities.FieldReports, "-created_at", 1e3));
...
async function getOrCreateConfig(ctx) {
  ...
  return await ctx.entities.AgentOpsConfig.create({
    config_key: "autonomous_ops",
    autonomous_enabled: true,
    paused: false,
    lead_enablement: DEFAULT_ENABLEMENT,
    scheduled_time: "07:00"
  });
}
async function executeDailyRuns(ctx, { triggeredBy = "scheduler", onlyLeadId = null } = {}) {
  ...
  const runCtx = { ...ctx, now: runNow, dailyReadCache: /* @__PURE__ */ new Map() };
  ...
  for (const lead of leads) {
    const run = await ctx.entities.AgentDailyRun.create({
      ...
      blockers: [],
      exceptions: [],
      plan_snapshot: lead.plan,
      escalated: false
    });
    let result;
    try {
      result = await reconcileLead(lead, runCtx);
    } catch {
      result = { summary: "Reconciliation failed; internal data access needs review.", exceptions: ["Connector failure or data unavailable prevented reconciliation."] };
    }
  ...
  for (const r of results) {
    if (r.state === "blocked" && r.exceptions && r.exceptions.length > 0) {
      ...
      r.escalated = true;
      r.escalation_id = esc.id;
      escalated++;
    }
  }
  await ctx.entities.AgentOpsConfig.update(config.id, { last_run_at: (/* @__PURE__ */ new Date()).toISOString() });
  return {
    ran: results.length,
...
// base44/shared/jobKnowledgeDaily.ts
async function prepareDailyKnowledge(base44, { triggeredBy, onlyLeadId } = {}) {
...
Deno.serve(async function(req) {
  try {
    ...
    const ctx = { entities: base44.asServiceRole.entities, connectors: base44.asServiceRole.connectors, secrets: secrets2 };
