...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(query.brief===true)return Response.json({range_start:start,range_end:end,counts:result.counts,complete_result:true,source_status:{tracker_captured_at:tracker.source_captured_at,outlook_captured_at:snapshot.captured_at,probuild_reports_loaded:reports.length,linked_reports:result.events.reduce((n,e)=>n+e.reports.length,0)},warnings:result.events.filter(e=>e.warnings.length).map(e=>({job_name:e.job_name,event_date:e.event_date,warnings:e.warnings})).slice(0,10),warning_event_count:result.events.filter(e=>e.warnings.length).length,note:"Counts cover the full requested range. Unmatched items are retained for review, not deleted. Arrival estimates are separate from installations. No completion is inferred. This compares stored sources; it does not refresh them."});
