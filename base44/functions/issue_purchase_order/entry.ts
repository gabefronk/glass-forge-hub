...
export default async function(req) {
  try {
    ...
    if (body.review_confirmed !== true) {
      return Response.json({ error: 'owner_review_required' }, { status: 400 });
    }
    ...
    // Job setup approval supplies identity context only. Never read its nested costs,
    // pricing, sources, or scope as vendor-specific PO values.
    const sheets = await fetchAllPages(api.JobSetupSheets, 'created_date').catch(() => []);
    ...
    const setupSheet = approvedSheets[0];
    ...
    const row = {
      po_number, status, created_by_email: user.email || '',
    ...
    row.job_id = jobId;
    row.job_name = clean(setupSheet.job_name) || clean(job.canonical_name);
