...
test('owner gate is identical in UI and server and accepts both verified Gabriel accounts only',()=>{for(const [user,yes] of [[null,false],[{email:'iryedra@gmail.com',role:'admin'},false],[{email:'trevor.draney7@gmail.com',role:'admin'},false],[{email:'gabefronk@gmail.com',role:'user'},false],[{email:'gabefronk@gmail.com',role:'admin'},true],[{email:'gabriel.fronk.wd@gmail.com',role:'admin'},true]]){assert.equal(isAgentCenterOwner(user),yes);assert.equal(uiOwner(user),yes)}});
test('tree includes the manager and configured-but-idle Claude quoting agent',()=>{const nodes=buildAgentInventory({now});assert.equal(nodes.find(x=>x.id==='manager_agent').parent_id,null);const quoting=nodes.find(x=>x.id==='construction_plan_quoting');assert.equal(quoting.parent_id,'manager_agent');assert.equal(quoting.connection,'configured');assert.equal(quoting.provider,'Anthropic Claude');assert.match(quoting.assignment,/pricing-engine-only/);});
...
test('connection setup stores only a secret name and test remains planned without a real secret',async()=>{const h=harness();const prep=await h.call({action:'connection_prepare',provider_id:'anthropic_claude',scope_summary:'Status only',auth_method:'api_key'});assert.equal(prep.status,200);assert.equal(h.connections[0].connection_state,'planned');assert.equal(h.connections[0].secret_reference,'ANTHROPIC_API_KEY');assert.ok(!JSON.stringify(h.connections).match(/key_value|secret_value|password/i));const status=await h.call({action:'connection_status',provider_id:'anthropic_claude'});assert.equal(status.body.configured,false);assert.match(status.body.message,/ANTHROPIC_API_KEY/);const tested=await h.call({action:'connection_test',provider_id:'anthropic_claude'});assert.equal(tested.body.verified,false);assert.match(tested.body.message,/ANTHROPIC_API_KEY/);const off=await h.call({action:'connection_disconnect',provider_id:'anthropic_claude'});assert.equal(off.status,200);assert.equal(h.connections[0].connection_state,'disconnected');});
...
test('text collector reports failures and stale heartbeats instead of claiming online',()=>{
 const device={label:'Mac service draft collector',enabled:true,last_seen_at:now.toISOString()};
 const status=d=>buildAgentInventory({now,messageDevices:[d]}).find(n=>n.id==='message_service_assistant');
 assert.equal(status(device).status,'Draft collector online');
 assert.equal(status({...device,last_error:'Source unavailable'}).status,'Collector needs attention');
 assert.equal(status({...device,last_seen_at:'2026-09-09T12:00:00Z'}).status,'Waiting for Mac collector');
 assert.equal(status({...device,enabled:false}).status,'Collector paused');
});
