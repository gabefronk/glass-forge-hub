...
function setup(user={role:'admin',email:'gabefronk@gmail.com'}) {
  ...
  const entities=new Proxy({}, {get:(_t,name)=>({list:async()=>{calls.push(name+'.list');assert.equal(name,'Jobs');return jobs;},filter:async()=>{calls.push(name+'.filter');assert.equal(name,'JobKnowledgeRun');return [];},create:()=>assert.fail('write'),update:()=>assert.fail('write'),delete:()=>assert.fail('write')})});
...
test('research endpoint uses exact saved identity and no provider, model or record writes',async()=>{
  ...
  assert.deepEqual(h.calls,['Jobs.list','JobKnowledgeRun.filter']);
});
test('conflicting order never causes a preparation read or source research',async()=>{
  ...
  assert.equal(r.data.lookup.status,'conflict');assert.equal(r.data.research_plan.status,'blocked');assert.equal(r.data.research_plan.steps.length,0);assert.deepEqual(h.calls,['Jobs.list']);
});
test('fully specified missing lot can be researched provisionally without creating a job',async()=>{
  ...
  assert.equal(r.data.research_plan.status,'provisional_lookup');assert.equal(r.data.research_plan.identity.job_id,null);assert.equal(r.data.research_plan.auto_attach_to_job,false);assert.deepEqual(h.calls,['Jobs.list']);
});
