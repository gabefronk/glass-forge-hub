...
export const GLASS_CONSTRUCTION_VERSION = 'pk361-minimum-glass-v2';
...
export function minimumStandardGlass(width, height, tempered = false) {
  ...
  // PK105 R1473/R413 retain the 3/16 answer through 6,912 in² of
  // actual glass (48 ft²), inclusive. Larger constructions stay unported.
  if (!(w > 0 && h > 0) || Math.max(w,h) > 120 || area > 48 * 144) return null;
...
export function sourceGlassConstruction(c) {
  ...
  // Extend only the reviewed rectangular Studio Direct Set recipe. Its limit
  // applies to the glass pane below, rather than the surrounding frame area.
  if (w * h > 36 * 144 && !(c.series === 'Studio' && c.product_type === 'Direct Set')) return null;
  let panes;
