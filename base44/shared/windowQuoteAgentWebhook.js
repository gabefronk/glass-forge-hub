...
export function createAgentWebhookHandler({ getClient, execution, transport, secret }) {
  return async req => {
    ...
    try {
      ...
      const query = { id: quoteId, 'agent_run.conversation_id': event.conversation_id, execution_provider: 'superagent' };
      let rows = await db.QuoteRequests.filter(query, undefined, 2);
      if (!rows.length && db.WindowQuoteOnlineRequests) rows = await db.WindowQuoteOnlineRequests.filter(query, undefined, 2);
