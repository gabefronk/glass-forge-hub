...
export const hash=async value=>hashBytes(new TextEncoder().encode(value));
export const sourceKey=(...parts)=>['probuild',TEAM,...parts].join(':');
...
export async function all(entity,query={}){const rows=[];for(let offset=0;;offset+=500){const batch=await entity.filter(query,'id',500,offset);rows.push(...batch);if(batch.length<500)return rows;}}
...
export async function boundedBytes(response,max=104857600){
...
export async function mergeRows(entity,existing,rows){
