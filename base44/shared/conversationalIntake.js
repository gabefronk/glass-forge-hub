...
Single Hung already specifies its operation: do not add options.operation: 'Single Hung' or duplicate that style in another option field. For a confirmed standard request with no tempering or safety-glass notes, the standard non-tempered selection is already chosen; do not ask whether any windows require tempered glass. Explicit tempering, safety-glass requirements, uncertain safety needs or location concerns still require preservation and clarification as appropriate.
...
export function createConversationalIntake({ invokeLLM, normalizeStructured, timeoutMs = 25000 } = {}) {
  ...
  return async function normalizeIntake(q, runtimeContext = {}) {
    ...
    const resolvedTemperingQuestion = question => {
      if (!standardConfirmed(candidate) || !normalizedQuote.lines.length || !/\btemper(?:ed|ing)?\b/i.test(question)) return false;
      // Omitted routine tempering is selected by the confirmed recipe. A real
      // safety/location concern or conflicting customer note is not an omission.
      const safetyNotes = customerOptionNotes + '\n' + normalizedQuote.lines.map(line => line.room || '').join('\n');
      if (/\b(?:temper(?:ed|ing)?|safety|hazard|code|bath(?:room|tub)?|shower|stair(?:s|way)?|door(?:way)?|floor|pool)\b/i.test(safetyNotes) ||
          /\b(?:uncertain|unsure|not sure|don't know|do not know|do not assume|don't assume|do not use defaults|don't use defaults|never assume)\b/i.test(customerOptionNotes)) return false;
      if (/\b(?:conflict|contradict|different|instead|uncertain|unsure|privacy|obscure|pattern|tint|coating|coze|low[ -]?e|quantity|qty|width|height|dimensions?|sizes?|styles?|colou?rs?|operation|opening\s+direction|fin|exterior|interior|screens?|hardware|thickness|construction|glazing|argon|gas|spacers?|capillary|grilles?|grids?|elevation|sash|unit\s+type)\b/i.test(question)) return false;
      return normalizedQuote.lines.every((line, index) => typeof (line.options?.tempered ?? normalizedQuote.settings?.tempered) === 'boolean' &&
        !plannerIssues.some(item => item.path === 'lines[' + index + '].options.tempered' || item.path === 'settings.tempered'));
    };
    ...
    const proposedQuestions = (interpretation.questions.length ? [...interpretation.clarification, ...interpretation.questions] : [...interpretation.clarification, ...missingLines, ...missingPlanner])
      .filter(question => (!needsBasis || !basisQuestion.test(question)) &&
        (!needsFin.length || !/\b(?:fin|installation style|installation series)\b/i.test(question)) && !redundantAccountNumber(question) && !resolvedRoutineQuestion(question) && !resolvedAncillaryQuestion(question) && !resolvedTemperingQuestion(question));
