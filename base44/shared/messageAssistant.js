...
export function createMessageAssistantHandler({getClient,loadDirectory,now=()=>new Date()}){
 return async req=>{
  ...
  try{
   ...
   const raw=await req.text();if(raw.length>3000000)return reply({error:'Capture exceeds the request limit.'},413);
   const input=JSON.parse(raw),action=input.action,at=now().toISOString();
   if(device&&!['catalog','capture','analyze','heartbeat'].includes(action))return reply({error:'This action is not available to the collector.'},403);
   ...
   const builderKeys=new Set(contacts.map(c=>c.builder_key));
   const candidates=directory.jobs.filter(j=>builderKeys.has(j.builder_key)||contacts.some(c=>c.job_ids.includes(j.id)));
