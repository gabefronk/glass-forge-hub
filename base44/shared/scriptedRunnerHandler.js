...
export function createScriptedRunnerHandler({ getClient, execution, getExecution }) {
  ...
  return async req => {
    ...
    try {
      ...
      if (!body || typeof body !== 'object' || Array.isArray(body) || !['claim', 'heartbeat', 'checkpoint', 'report'].includes(body.action)) throw new HttpError(400, 'Unknown scripted runner action');
