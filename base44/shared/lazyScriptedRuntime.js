...
export function createLazyScriptedRuntime({ normalizeRequest, normalizeIntake, validateReady, fallbackExecution = null, packageOnlineExecution = null, loadConfig = loadScriptedRunnerConfig, hash, now, uuid } = {}) {
  async function resolve({ db }) {
    const config = await loadConfig({ db });
    const create = config.mode === 'queue' ? createScriptedQueueExecution : createScriptedExecution;
    const scripted = create({ config, normalizeRequest, normalizeIntake, validateReady, ...(config.mode === 'queue' ? { fallbackExecution, packageOnlineExecution } : {}), ...(hash ? { hash } : {}), ...(now ? { now } : {}), ...(uuid ? { uuid } : {}) });
