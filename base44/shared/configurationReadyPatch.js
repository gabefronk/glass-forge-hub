...
export async function configurationReadyPatch({ q, verified, work, config, policy = config?.native_engine, checkedAt = new Date().toISOString(), hash = sha256 }) {
  ...
  const hasSource = verified.some(row=>row?.source==='amsco_source_engine');
  const selectionHashes = await Promise.all(q.lines.map((line, index) => work ? hash(stable({ package_id: work.id, input_hash: work.input_hash, index, settings: q.settings, line })) : null));
    ...
    const lines = q.lines.map((requested, index) => {
      ...
      const source = row.source === 'amsco_source_engine';
      if (source && sourcePriceEvidenceIssues({...row.source_evidence,source:'amsco_source_engine'},observed,requested,q.settings).length) fail(502,'The calculated source price no longer matches this window');
      ...
      return { id: requested.id, source_index: index, ...(requested.mark !== undefined ? { mark: requested.mark } : {}),
        ...
        pricing_evidence: source ? { ...clone(row.source_evidence), source:'amsco_source_engine' } : online ? { ...clone(row.online_evidence), source: 'amsco_online' } : { version: 1, preview_id: row.id, plan_hash: row.plan_hash, native_quote_id: row.result.native_quote_id,
    ...
    const result = { schema_version: 1, native_source: CONFIGURATION_QUOTE_SOURCE, verified: true, quote_id: q.id, input_revision: q.input_revision, input_snapshot_sha256: inputHash,
      ...
      verification: { version: 1, source: CONFIGURATION_QUOTE_SOURCE, reopened: !hasSource, dimension_source: hasSource ? 'source_or_saved_frame' : 'saved_native_frame', ...(hasSource ? {source_price_version:SOURCE_PRICE_VERSION} : {}), checked_at: checkedAt,
    ...
    const content = hasSource ? 'Your quote is ready. Supported windows were calculated directly from the AMSCO pricebook. Any remaining windows use their verified AMSCO configuration prices.' : 'Your quote is ready. The total uses the saved and verified AMSCO price for each window.';
