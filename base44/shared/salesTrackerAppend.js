...
function rowDecision(existing, candidate) {
  ...
  const matches = existing.filter(row => trackerOrderKey(row) === key);
...
export function planTrackerAppend(existing, incoming) {
  ...
  const accepted = [], decisions = [], current = [...existing];
  for (let index = 0; index < incoming.length; index++) {
    const row = incoming[index], key = trackerOrderKey(row);
    const peers = incoming.filter(other => trackerOrderKey(other) === key && location(other) === location(row));
    const conflictingPeers = key && new Set(peers.map(fingerprint)).size > 1;
    const sameOrder = key ? incoming.filter(other => trackerOrderKey(other) === key) : [];
    ...
    if (decision.action === 'inserted') { accepted.push(row);current.push(row); }
  }
...
export function applyTrackerAppends(baseRows, batches) {
  const rows = [...baseRows], appended = [], conflicts = [], used = new Set();
  for (const batch of [...batches].sort((a,b)=>String(a.imported_at).localeCompare(String(b.imported_at)) || String(a.id).localeCompare(String(b.id)))) {
    ...
    for (const [i,original] of (batch.rows || []).entries()) {
      const decision = rowDecision(rows,original);
      ...
      rows.push(row);appended.push(row);
    }
  }
